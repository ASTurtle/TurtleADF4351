/*******************************************/
/* lcd.h 1.14寸 IPS LCD屏驱动头文件 */
/*ZJY 1.14寸 IPS LCD屏驱动*/
/*2025/3  UESTC AutoKx  Turtle*/
/***************************************** */
...
#define USE_PWM_BLK 1 //使用PWM控制背光灯
#define BLK_PNP 1  //使用PNP管驱动背光灯
//注：如果使用PWM控制背光灯，需要根据三极管类型(PNP/NPN)到CubeMX中修改PWM输出极性
...
// #define LCD_SCLK_Clr() HAL_GPIO_WritePin(LCD_SCLK_GPIO_Port,LCD_DC_Pin,GPIO_PIN_RESET)//SCL=SCLK  
// #define LCD_SCLK_Set() HAL_GPIO_WritePin(LCD_SCLK_GPIO_Port,LCD_DC_Pin,GPIO_PIN_SET)

// #define LCD_MOSI_Clr() HAL_GPIO_WritePin(LCD_DC_GPIO_Port,LCD_DC_Pin,GPIO_PIN_RESET)//SDA=MOSI
// #define LCD_MOSI_Set() HAL_GPIO_WritePin(LCD_DC_GPIO_Port,LCD_DC_Pin,GPIO_PIN_SET)
...
#if USE_PWM_BLK==1
#include "tim.h"
#define LCD_BLK_TIM htim1 //LCD背光灯PWM定时器
#define LCD_BLK_CHANNEL TIM_CHANNEL_2 //LCD背光灯PWM通道
#define LCD_BLK_PWM_Set(x) __HAL_TIM_SetCompare(&LCD_BLK_TIM,LCD_BLK_CHANNEL,x)//设置LCD背光灯驱动PWM占空比x%
#define LCD_BLK_PWM_Init() HAL_TIM_PWM_Start(&LCD_BLK_TIM,LCD_BLK_CHANNEL)//开启LCD背光灯驱动PWM
#define LCD_BLK_PWM_Stop() HAL_TIM_PWM_Stop(&LCD_BLK_TIM,LCD_BLK_CHANNEL)//关闭LCD背光灯驱动PWM
#endif



void LCD_Writ_Bus(uint8_t dat);//硬件SPI发送
